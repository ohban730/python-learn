...
| **数理の前提知識** | [odds_ratio_demo.py](./basics/odds_ratio_demo.py)<br>[gradient_descent_demo.py](./basics/gradient_descent_demo.py)<br>[variance_compatibility_demo.py](./basics/variance_compatibility_demo.py) | [odds_ratio.md](./docs/odds_ratio.md)<br>[gradient_descent.md](./docs/gradient_descent.md)<br>[variance_and_compatibility.md](./docs/variance_and_compatibility.md) | 「オッズ比」の数学的定義、損失等高線上のパラメータ探索軌跡、および「有偏・不偏分散（ベッセル補正）」の定義と公式モデルとのパラメータロード互換性 |
...
| **Step 5: GPTモデルの構築** | - | [gpt_architecture.md](./docs/gpt_architecture.md) | GPTモデルの全体設計・実装ロードマップ（プレースホルダ設計）から、入出力テンソルの全体予測フロー、ブロック内部の計算パスまでを可視化 |
