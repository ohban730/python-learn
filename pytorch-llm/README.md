...
│   ├── loss_calculation_demo.py    # -> 損失計算の6ステップ(図5-7)の実証シミュレータ
│   ├── perplexity_demo.py          # -> パープレキシティ(PPL)の数理検証シミュレータ
│   ├── training_loop_demo.py       # -> 訓練ループ(zero_grad/backward/step)の実証シミュレータ
│   ├── optimizer_demo.py           # -> AdamとAdamWの数理・更新挙動の違いの実証シミュレータ
│   └── multinomial_demo.py         # -> 確率サンプリング(multinomial)の実証シミュレータ
│
...
└── docs/                 # 📚 ビジュアル解説ドキュメント & 用語集
    ...
    ├── cross_entropy_loss_calculation.md # -> 損失計算の6ステップ(図5-7)の数理解説
    ├── perplexity_basics.md        # -> パープレキシティ(PPL)の数理モデル解説
    ├── pytorch_training_loop.md    # -> 訓練ループとエポック・バッチの数理解説
    ├── optimizer_adam_vs_adamw.md  # -> オプティマイザ(Adam vs AdamW)の数理的差異の解説
    └── multinomial_sampling.md     # -> 確率サンプリング(multinomial)の数理解説
```
...
| **Step 5: GPTモデルの構築** | [loss_calculation_demo.py](./basics/loss_calculation_demo.py)<br>[perplexity_demo.py](./basics/perplexity_demo.py)<br>[training_loop_demo.py](./basics/training_loop_demo.py)<br>[optimizer_demo.py](./basics/optimizer_demo.py)<br>[multinomial_demo.py](./basics/multinomial_demo.py) | [gpt_architecture.md](./docs/gpt_architecture.md)<br>[final_normalization_and_output_head.md](./docs/final_normalization_and_output_head.md)<br>[weight_tying_basics.md](./docs/weight_tying_basics.md)<br>[generation_loop_stopping.md](./docs/generation_loop_stopping.md)<br>[cross_entropy_loss_calculation.md](./docs/cross_entropy_loss_calculation.md)<br>[perplexity_basics.md](./docs/perplexity_basics.md)<br>[pytorch_training_loop.md](./docs/pytorch_training_loop.md)<br>[optimizer_adam_vs_adamw.md](./docs/optimizer_adam_vs_adamw.md)<br>[multinomial_sampling.md](./docs/multinomial_sampling.md) | GPTモデルの全体設計・実装ロードマップ（プレースホルダ設計）から、入出力テンソルの全体予測フロー、ブロック内部の計算パス、最終出口のLayerNormと線形出力層、「重み共有」、EOSトークン・最大長・Stop Sequencesによる生成ループの停止メカニズム、図5-7の損失(NLL)計算の6つの数理ステップ、パープレキシティの検証、図5-11の一般的な訓練ループ（Epoch, Batch, Step）の検証、オプティマイザ（Adam vs AdamW）の数理的差異の実証、および確率比例サンプリング（multinomial）による文章生成の多様性制御までを網羅 |
